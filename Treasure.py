...
def drawBoard(board):
    ...
    hline= '   ' # initial spave for the numbers down the left side of the board
    for i in range(1, 6):
        hline += (' ' * 9) + str(1)
...
def getNewBoard():
    #Create a new 60*15 board data structurn.
    board=[]
    for x in range(60):#the main list is a list of 60 lists
        board.append([])
        for y in range(15):#each list in tne main list has 15 single-character strings
...
def isValidMove(x,y):
    #Return Ture if the coordinates are on the board, otherwise False.
    return x >= 0 and x <= 59 and y>= 0 and y<= 14

def fmakeMove(board, chests, x ,y):
    #Change the chests list as structrue with a sonar device character. Remove treasure chests
    #from the chests list as they ar fround. Return False if this is an invalid move.
    ...
    for cx, cy in chests:
        ...
        if distance < smallestDistance:#we want the cloest treasure chest.
...
is to find the three sunken treasure chests that sre lurking in the part of the
...
represt distances of 2 away from the device. The 4's represt
...
while Ture:
    ...
    while sonarDevices>0:
        ...
        print('You have %s sonar device%s left. %s treasure chest%s remaining.' % (SonarDevices, extraSsonat, len(theChests),extraSchest))

        x ,y = enterPlayerMove()
        previousMoves.append([x,y])#we must track all move so that sonar devices can be updated.
